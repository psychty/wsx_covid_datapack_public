...
var svg_fg_age_stack_1 = d3
  .select("#my_lifecourse_condition_dataviz")
  .append("svg")
...
request.open(
  "GET",
  "./Numbers_lifecourse_persons_level_2_2017_west_sussex.json",
  false
);
request.send(null);
...
request.open(
  "GET",
  "./Numbers_lifecourse_persons_level_2_2017_west_sussex_stack_value_max.json",
  false
);
request.send(null);
...
var showTooltip_age = function (d, i) {
  ...
  tooltip_age
    .html(
      "<h3>" +
        sub_cause_groupName +
        '</h3><p class = "tooltip_b">The estimated number of ' +
        label_key(d.data.Measure) +
        " as a result of " +
        sub_cause_groupName.toLowerCase() +
        " in West Sussex in 2017 among both males and females aged " +
        d.data.Age +
        ' was <font color = "#1e4b7a"><b>' +
        d3.format(",.0f")(subgroupValue) +
        '</b></font>.</p><p class = "tooltip_b">This is <font color = "#1e4b7a"><b>' +
        d3.format(",.0%")(subgroupValue / d.data.Total_in_age) +
        "</b></font> of the total " +
        label_key(d.data.Measure) +
        " in West Sussex among those aged " +
        d.data.Age +
        ' (<font color = "#1e4b7a"><b>' +
        d3.format(",.0f")(d.data.Total_in_age) +
        "</b></font>)</p>"
    )
    .style("opacity", 1)
...
// // Add Y axis
var y_fg_4 = d3.scaleLinear().domain([0, 2000]).range([height_ages, 0]);

var yAxis_fg_4 = svg_fg_age_stack_1.append("g").attr("class", "myYaxis");
...
function update_age(e) {
  ...
  svg_fg_age_stack_1.selectAll("#age_stack_label_1").remove();

  svg_fg_age_stack_1.selectAll("#age_stack_label_2").remove();

  svg_fg_age_stack_1.selectAll("rect").remove();
  ...
  var selected_stack = function (d) {
    ...
    values_selected = json_max.filter(function (d) {
      return (
        (d.Cause === sub_cause_groupName) & (d.Measure === measure_name[0])
      );
    });

    y_fg_4.domain([0, values_selected[0].Estimate_max]);

    // This adds a transition effect on the change between datasets (i.e. if the yaxis needs to be longer or shorter).
    yAxis_fg_4.transition().duration(1000).call(d3.axisLeft(y_fg_4));
    ...
    svg_fg_age_stack_1.selectAll("#age_stack_label_3").remove();
    ...
    setTimeout(function () {
      svg_fg_age_stack_1
        ...
        .attr("class", function (d, i) {
          return "myRect" + i;
        }) // Add an id to each subgroup: their name
        .selectAll("rect")
        .data(function (d) {
          return d;
  ...
  var restore_stacks = function (d) {
    ...
    y_fg_4.domain([0, figure_4_y_max]);
  ...
  var stackedData = d3.stack().keys(cause_categories)(data);
...
var svg_fg_age_stack_2 = d3
  .select("#my_condition_lifecourse_dataviz")
  .append("svg")
  .attr("width", width_ages)
...
request.open(
  "GET",
  "./Numbers_lifecourse_persons_by_condition_level_2_2017_west_sussex.json",
  false
);
request.send(null);
var json = JSON.parse(request.responseText); // parse the fetched json data into a variable
...
request.open(
  "GET",
  "./Numbers_lifecourse_persons_conditions_level_2_2017_west_sussex_stack_value_max.json",
  false
);
request.send(null);
...
var showTooltip_condition_age = function (d) {
  tooltip_condition_age.transition().duration(200);
  ...
  tooltip_condition_age
    .html(
      "<h3>" +
        ...
        "</h3><h5>" +
        d.data.Cause +
        '</h5><p class = "tooltip_b">The estimated number of ' +
        label_key(d.data.Measure) +
        " as a result of " +
        d.data.Cause +
        " among those aged " +
        subgroupName +
        ' in West Sussex in 2017 was <font color = "#1e4b7a"><b>' +
        d3.format(",.0f")(subgroupValue) +
        '</b></font>.</p><p class = "tooltip_b">This is <font color = "#1e4b7a"><b>' +
        d3.format(",.0%")(subgroupValue / d.data.Total_in_condition) +
        "</b></font> of the total " +
        label_key(d.data.Measure) +
        ' in West Sussex from this cause (<font color = "#1e4b7a"><b>' +
        d3.format(",.0f")(d.data.Total_in_condition) +
        "</b></font>)</p>"
    )
    .style("opacity", 1)
    .style("top", event.pageY - 10 + "px")
...
// // Add Y axis
var y_fg_5 = d3.scaleLinear().domain([0, 3000]).range([height_ages, 0]);

var yAxis_fg_5 = svg_fg_age_stack_2.append("g").attr("class", "myYaxis");
...
function update_condition(e) {
  svg_fg_age_stack_2.selectAll("rect").remove();

  svg_fg_age_stack_2.selectAll("#age_stack_label_1").remove();

  svg_fg_age_stack_2.selectAll("#age_stack_label_2").remove();
  ...
  var selected_age_stack = function (d) {
    ...
    age_values_selected = json_age_max.filter(function (d) {
      return (
        (d.Age === sub_age_groupName) & (d.Measure === measure_name_fg_5[0])
      );
    });
    ...
    y_fg_5.domain([0, age_values_selected[0].Estimate_max]);

    // This adds a transition effect on the change between datasets (i.e. if the yaxis needs to be longer or shorter).
    yAxis_fg_5.transition().duration(1000).call(d3.axisLeft(y_fg_5));
    ...
    svg_fg_age_stack_2.selectAll("#age_stack_label_3").remove();
  ...
  var restore_age_stacks = function (d) {
    ...
    y_fg_5.domain([0, figure_5_y_max]);
  ...
  var stackedData = d3.stack().keys(ages)(data);
  ...
  var condition_bars_df = svg_fg_age_stack_2
    .append("g")
    .selectAll("g")
    .data(stackedData) // Enter in the stack data = loop key per key = group per group
    .enter()
