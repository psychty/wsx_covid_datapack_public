...
var svg_age_spec_1 = d3
  .select("#age_spec_fig_1")
  .append("svg")
  .attr("width", width_hm)
...
request.open(
  "GET",
  "./Outputs/age_specific_rates_10_years_by_date.json",
  false
);
request.send(null);
...
request.send(null);
var dates_asr_ticks = JSON.parse(request.responseText).map(function (d) {
  return d.Date_label;
});

// Retrieve the selected area name
var selected_age_line_1_area_option = d3
  .select("#select_lines_age_1_area_button")
  .property("value");

// Update text based on selected area
d3.select("#selected_age_line_1_compare_title").html(function (d) {
  return (
    "Covid-19 pillar 1 and 2 confirmed age-specific rolling 7 day case rate (per 100,000 population) over time up to " +
    complete_date +
    "; " +
    selected_age_line_1_area_option +
    "; as at " +
    data_refreshed_date
  );
});
...
var dates_asr = d3
  .map(age_spec_1_chosen, function (d) {
    return d.Date_label;
  })
  .keys();
...
// Group the data
var age_spec_1_chosen_ts_group = d3
...
y_asr_1_ts = d3
  ...
  .domain([
    0,
    d3.max(age_spec_1_chosen, function (d) {
      return +d.ASR;
    }),
  ])
...
y_asr_1_ts_axis.selectAll("text").style("font-size", ".8rem");

var tooltip_asr_1_dot = d3
  .select("#age_spec_fig_1")
  .append("div")
  .style("opacity", 0)
  ...
  tooltip_asr_1_dot
    .html(
      "<h4>" +
        ...
        " - " +
        d.Name +
        '</h4><p class = "side">The number of cases among those aged ' +
        d.Age +
        ...
        "</b>. This is " +
        d3.format(",.0f")(d.ASR) +
        ' per 100,000 population.</p><p class = "side">So far, there have been a total of ' +
...
var linehover_asr_1 = function (d) {
  tooltip_asr_1_dot
    .html(
      "<h4>" +
        d.key +
        '</h4><p class = "side">Hover over a dot to find out more.</p>'
    )
    .style("opacity", 1)
    .style("top", event.pageY - 10 + "px")
    .style("left", event.pageX + 10 + "px")
...
var mouseleave_asr_1 = function (d) {
  tooltip_asr_1_dot.style("visibility", "hidden");
...
var lines_asr_1 = svg_age_spec_1
  ...
  .attr("d", function (d) {
    return d3
      .line()
      .x(function (d) {
        return x_asr_1(d.Date_label);
      })
      .y(function (d) {
        return y_asr_1_ts(+d.ASR);
      })(d.values);
  })
...
function update_asr_1() {
  ...
  y_asr_1_ts_axis.transition().duration(1000).call(d3.axisLeft(y_asr_1_ts));
