...
d3.select("#pathways_title").html(function (d) {
  return (
    "Total number of complete triages to NHS Pathways for COVID-19; NHS West Sussex CCG" +
    "; 19 March - " +
    latest_triage_date
  );
});

var svg_pathways_fig = d3
  .select("#pathways_fig")
  .append("svg")
  .attr("width", width_hm)
...
latest_p_summary = pathways_df.filter(function (d) {
  return d.Date === pathway_dates[pathway_dates.length - 1];
})[0];
...
xAxis_pathways
  ...
  .attr(
    "transform",
    "translate(-" + (x_pathways.bandwidth() + 10) + ",10)rotate(-90)"
  )
...
var y_pathways = d3
  ...
  .domain([
    0,
    d3.max(pathways_df, function (d) {
      return +d.Triage_count;
    }),
  ])
  .range([height_line - 80, 0])
  .nice();

var yAxis_pathways = svg_pathways_fig.append("g").call(d3.axisLeft(y_pathways));

yAxis_pathways.selectAll("text").style("font-size", ".8rem");

var tooltip_pathways = d3
  .select("#pathways_fig")
  .append("div")
  .style("opacity", 0)
...
var showTooltip_pathways = function (d) {
  tooltip_pathways
    .html(
      "<h5>" + d.Date + "</h5><p>" + d.label_1 + "</p><p>" + d.label_2 + "</p>"
    )
    .style("opacity", 1)
...
var mouseleave_pathways = function (d) {
  tooltip_pathways.style("opacity", 0).style("visibility", "hidden");
};
...
svg_pathways_fig
  ...
  .attr("x", function (d) {
    return (
      x_pathways(pathway_dates[pathway_dates.length - 5]) +
      x_pathways.bandwidth() / 2
    );
  })
  .attr("y", 25)
...
d3.select("#pathways_label_1").html(function (d) {
  return latest_p_summary.label_1;
});

d3.select("#pathways_label_2").html(function (d) {
