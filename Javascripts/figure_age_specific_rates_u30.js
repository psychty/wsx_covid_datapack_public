...
var selected_age_line_2_area_option = d3
  .select("#select_lines_age_2_area_button")
  .property("value");

// Update text based on selected area
d3.select("#selected_age_line_2_compare_title").html(function (d) {
  return (
    "Covid-19 pillar 1 and 2 confirmed age-specific rolling 7 day case rates (per 100,000 population) over time among under 30s up to " +
    complete_date +
    "; " +
    selected_age_line_2_area_option +
    "; as at " +
    data_refreshed_date
  );
});
...
var dates_u30_asr = d3
  .map(age_spec_u30_chosen, function (d) {
    return d.Date_label;
  })
  .keys();
...
// Group the data
var age_spec_u30_chosen_ts_group = d3
...
xAxis_asr_u30
  ...
  .attr(
    "transform",
    "translate(-" + (x_asr_u30.bandwidth() + 15) + ",10)rotate(-90)"
  )
...
y_asr_u30_ts = d3
  ...
  .domain([
    0,
    d3.max(age_spec_u30_chosen, function (d) {
      return +d.ASR;
    }),
  ])
...
var mouseleave_asr_u30 = function (d) {
  tooltip_asr_u30_dot.style("visibility", "hidden");
  ...
  lines_asr_u30
    ...
    .duration(500)
    .style("stroke", function (d) {
      return area_age_u30_colours(d.key);
    })
    .style("visibility", "visible")
    .attr("stroke-width", 2);
  ...
  dots_asr_u30
    ...
    .duration(500)
    .style("fill", function (d) {
      return area_age_u30_colours(d.Age);
    })
    .style("stroke", function (d) {
      return area_age_u30_colours(d.Age);
    })
...
var lines_asr_u30 = svg_age_spec_u30
  ...
  .attr("d", function (d) {
    return d3
      .line()
      .x(function (d) {
        return x_asr_u30(d.Date_label);
      })
      .y(function (d) {
        return y_asr_u30_ts(+d.ASR);
      })(d.values);
  })
...
function update_asr_u30() {
  ...
  y_asr_u30_ts_axis.transition().duration(1000).call(d3.axisLeft(y_asr_u30_ts));
