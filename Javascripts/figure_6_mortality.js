...
var colour_covid_non_covid_all_settings = d3.scaleOrdinal()
  .domain(covid_causes)
  .range(['#BDD7EE','#2F5597'])

var colour_covid_non_covid_carehomes = d3.scaleOrdinal()
  .domain(covid_causes)
  .range(['#FFD966', '#ED7D31'])
...
d3.select("#select_mortality_1_area_button")
  .selectAll('myOptions')
  .data(['West Sussex', 'Adur', 'Arun', 'Chichester', 'Crawley','Horsham','Mid Sussex','Worthing'])
  .enter()
  .append('option')
  .text(function(d) {
    return d;
...
tooltip_m1
  .html("<h5>" + d.data.Name + '</h5><p class = "side">Week number ' + d.data.Week_number + ' - ' + d.data.Date_label + '</p><p><b>'+ causeName + '</b></p><p><b>' + d3.format(',.0f')(causeValue) + ' deaths </b>' + attribute_label(causeName) + '</p><p class = "side">' + d.data.Deaths_in_week_label + '</p><p class = "side">' + d.data.Cumulative_deaths_label + '</p>' )
  .style("opacity", 1)
...
var x_m1 = d3.scaleBand()
  .domain(weeks)
  .range([0, width_hm_mort * .6]) // this is the 50 that was pushed over from the left plus another 10 so that the chart does not get cut off
  .padding([0.2]);
...
var bars_m1 = svg_fg_mortality_1
 ...
 .data(stackedData_m1)
 .enter().append("g")
 .attr("fill", function(d) { return colour_covid_non_covid_all_settings(d.key); })
 .selectAll("rect")
...
svg_fg_mortality_1
  ...
  .attr("x", function(d) {
    if ( chosen_latest[0]['Cumulative_covid_deaths'] > 10000){
      return width_hm_mort * .65 + 75 }
      else if ( chosen_latest[0]['Cumulative_covid_deaths'] > 1000) {
      return width_hm_mort * .65 + 65 }
      else if ( chosen_latest[0]['Cumulative_covid_deaths'] > 100) {
      return width_hm_mort * .65 + 45 }
      else {
      return width_hm_mort * .65 + 35
    }})
  .attr("y", 70)
  .attr("text-anchor", "start")
  .style('font-size', '10px')
...
// Create a tooltip for the lines and functions for displaying the tooltips as well as highlighting certain lines.
var tooltip_m2 = d3.select("#covid_non_covid_mortality_carehomes")
  .append("div")
  .style("opacity", 0)
  .attr("class", "tooltip_class")
  .style("position", "absolute")
...
tooltip_m2
  .html("<h5>" + d.data.Name + '</h5><p class = "side">Week number ' + d.data.Week_number + ' - ' + d.data.Date_label + '</p><p><b>'+ causeName + '</b></p><p> <b>' + d3.format(',.0f')(causeValue) + ' deaths in care homes </b>' + attribute_label(causeName) + '</p><p class = "side">' + d.data.Deaths_in_week_label + '</p><p class = "side">' + d.data.Cumulative_deaths_label + '</p>' )
  .style("opacity", 1)
  .attr('visibility', 'visible')
  .style("top", (event.pageY - 10) + "px")
  .style("left", (event.pageX + 10) + "px")
...
// append the svg object to the body of the page
var svg_fg_mortality_2 = d3.select("#covid_non_covid_mortality_carehomes")
 .append("svg")
  .attr("width", width_hm_mort)
  .attr("height", height_line_mort)
  .append("g")
...
var bars_m2 = svg_fg_mortality_2
 ...
 .data(stackedData_m2)
 .enter().append("g")
 .attr("fill", function(d) { return colour_covid_non_covid_carehomes(d.key); })
 .selectAll("rect")
 .data(function(d) { return d; })
 .enter().append("rect")
  .attr('id', 'bars2')
...
svg_fg_mortality_2
    ...
    .attr("cx", (width_hm_mort * .65) - 20)
    .attr("cy", 72)
    .attr("r", 10)
    .attr("fill", function(d) { return colour_covid_non_covid_carehomes('Covid-19'); })
...
d3.select("#selected_m1_title")
  .html(function(d) {
    return 'Weekly deaths (all ages, all settings); ' + chosen_m1_area + '; ' + chosen_m1_df[0].Date_label + ' - ' + chosen_m1_df[chosen_m1_df.length-1].Date_label
  });

d3.select("#selected_m1_subtitle")
  .html(function(d) {
    return 'By week of occurrence and by COVID-19 mentioned on death certificate for deaths registered by ' + ons_mortality_figures_dates[0].Reported_week_ending
  });
...
svg_fg_mortality_1
    .selectAll("#m1_selected_cumulative_all_text_5")
    .remove();

svg_fg_mortality_1
  .append("text")
  .attr("x", (width_hm_mort * .65) - 25)
  .attr("y", 40)
  .attr('id', 'm1_chosen_area')
  .attr("text-anchor", "start")
  .text(chosen_m1_area)
...
svg_fg_mortality_1
    ...
    .attr("cy", 72)
    .attr("r", 10)
    .attr("fill", function(d) { return colour_covid_non_covid_all_settings('Covid-19'); })
...
d3.select("#selected_m2_title")
  .html(function(d) {
    return 'Weekly deaths (all ages, care home settings); ' + chosen_m1_area + '; ' + chosen_m2_df[0].Date_label + ' - ' + chosen_m2_df[chosen_m1_df.length-1].Date_label
  });

d3.select("#selected_m2_subtitle")
  .html(function(d) {
    return 'By week of occurrence and by COVID-19 mentioned on death certificate for deaths registered by ' + ons_mortality_figures_dates[0].Reported_week_ending
  });
...
svg_fg_mortality_2
    .selectAll("#covid_dot_m2")
    .remove();

svg_fg_mortality_2
    .selectAll("#m2_selected_cumulative_covid")
    .remove();

svg_fg_mortality_2
    .selectAll("#m2_place_1")
    .remove();

svg_fg_mortality_2
    .selectAll("#m2_place_2")
    .remove();

svg_fg_mortality_2
    .selectAll("#m2_place_3")
    .remove();

svg_fg_mortality_2
    .selectAll("#m2_selected_cumulative_all")
    .remove();

svg_fg_mortality_2
    .selectAll("#m2_selected_cumulative_all_text_1")
    .remove();

svg_fg_mortality_2
    .selectAll("#m2_selected_cumulative_all_text_2")
    .remove();

svg_fg_mortality_2
    .selectAll("#m2_selected_cumulative_all_text_3")
    .remove();

svg_fg_mortality_2
    .selectAll("#m2_selected_cumulative_all_text_4")
    .remove();
...
svg_fg_mortality_2
    .selectAll("#m2_selected_cumulative_all_text_5")
    .remove();
